...
app.post('/api/admin/reset-data', async (req, res) => {
    if (req.body.password === '456789') {
        ...
        await logActivity('Admin', 'DATABASE_RESET_SUCCESS');
...
app.post('/api/campaign/shoot', async (req, res) => {
    ...
    for (let t of targetList) {
        ...
        await Coupon.create({ 
            ...
            location: t.location, proPhone: t.proNumber, audienceType, expiryDate: new Date(expiryDate) 
        });
        const tMap = { 'Doctor_30': 'temp_doctor_30_dis_new', 'Patient_30': 'patient_30_temp_new_dis' };
        const tName = tMap[`${audienceType}_${discount}`] || 'temp_doctor_30_dis_new';
        await sendWatiMessage(t.phone, tName, [{name:'1', value: t.name || 'Doctor'}, {name:'2', value: code}, {name:'3', value: formattedDate}]);
    }
    res.json({ success: true });
});
